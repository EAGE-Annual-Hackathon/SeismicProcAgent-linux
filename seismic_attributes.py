...
mcp = FastMCP(
    name="seismic attributes server",
    instructions="""
        This server provides functionalities to compute seismic attributes.
        """,
)

# https://github.com/dudley-fitzgerald/d2geo/tree/master
...
@mcp.tool()
async def sliceAttribute(data_path: str, data: str, inline: int = 189, crossline: int = 193, output='2d', type='il', 
                   inline_loc=400, 
                   xline_loc=1000, 
                   timeslice_loc=1404, 
                   ...
                   display: bool = True, 
                   figsize=(15,7), cmap='plasma', vmin=None, vmax=None) -> Image:
  ...
  inline: inline location, usually 189 or 181
  crossline: crossline location, usually 193 or 185
  ...
  cube = openSegy3D(filename, inline, crossline)
  ...
  if output == '2d':
    ...
    if type == 'il':
      ...
      slices = sliceCube(cube, type, inline_loc=int(inline_loc))
      ...
      np.save(f"Attribute_{attribute_type}_{type}.npy", result.compute())
      ...
      if display==True:
        ...
        plt.imshow(reshape.T, vmin=vmin, vmax=vmax, aspect='auto', 
                   extent=extent, cmap=cmap)
        plt.colorbar()        

        out_file = f"Attribute_{attribute_type}_{type}.jpg"
    ...
    if type == 'xl':
      slices = sliceCube(cube, type, xline_loc=int(xline_loc))      
    ...
    if type == 'ts':
      slices = sliceCube(cube, type, timeslice_loc=int(timeslice_loc))      
      ...
      if display==True:
        ...
        plt.figure(figsize=figsize)
        plt.imshow(reshape, vmin=vmin, vmax=vmax, aspect='auto', 
                        extent=extent, cmap=cmap)
        plt.title(f"Attribute {attribute_type} {type}")
        plt.colorbar()
